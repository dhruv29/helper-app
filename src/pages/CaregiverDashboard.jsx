...
const APPOINTMENTS = [
  { title: 'Dr. Johnson — Primary Care', date: 'Tue, Apr 15', time: '2:00 PM', icon: '🩺', warm: true  },
  { title: 'Blood Panel Lab Work',        date: 'Thu, Apr 17', time: '9:00 AM', icon: '🔬', warm: false },
]
...
function Card({ children, className = '', delay = 0, style }) {
  return (
    <motion.div className={`${G.card} ${className}`} style={style} {...fade(delay)}>
      {children}
    </motion.div>
  )
}
...
function WellnessRing({ score }) {
  ...
  const glow  = score >= 80 ? 'rgba(16,185,129,0.45)' : 'rgba(245,158,11,0.45)'
...
const A_STYLES = {
  ...
  medium: { border: 'border-amber-400/45',         badge: 'amber',      label: 'REVIEW', pulse: false },
        ...
        <div className="relative z-10 p-5">
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              ...
              <div key={label}
                className={`rounded-xl p-3 text-center ${warn ? 'bg-amber-500/20 border border-amber-400/30' : 'bg-white/[0.07] border border-white/[0.05]'}`}>
                <p className={`text-xl font-black leading-none ${warn ? 'text-amber-300' : 'text-white'}`}>{value}</p>
                <p className="text-[9px] font-semibold mt-1 uppercase tracking-wide" style={{ color: warn ? undefined : 'rgba(255,255,255,0.30)' }}>{sub}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <Card delay={0.14} className="p-5">
                ...
                <div className="rounded-sm"
                  style={{
                    ...
                    background: isToday
                      ...
                      : 'rgba(224,122,82,0.28)',
                    boxShadow: isToday ? `0 0 12px rgba(224,122,82,0.45)` : 'none',
                  }} />
              </div>
            )
          })}
        ...
        <div className="flex">
          {['M','T','W','T','F','S','S'].map((d, i) => (
            <span key={i} className="flex-1 text-center text-[10px] font-semibold"
              style={{ color: i === 6 ? ACCENT : 'rgba(255,255,255,0.18)' }}>{d}</span>
          ))}
        </div>
      </Card>
        ...
        <div className="grid grid-cols-2 gap-3">
          {VITALS.map((v, i) => (
            <motion.div key={v.label}
              className={`${G.card} p-4 ${!v.good ? '!border-amber-500/30' : ''}`}
              {...fade(0.08 + i * 0.05)}>
          ...
          return (
            <div key={i} className={`px-5 py-4 flex items-center gap-4 ${i > 0 ? `border-t ${G.div}` : ''}`}>
              ...
              <div className="flex-1">
                ...
                <p className="text-xs font-semibold mt-1" style={{ color: tc }}>{apt.date} · {apt.time}</p>
              </div>
            </div>
          )
        })}
  ...
  return (
    <div className="space-y-5">
      ...
      <motion.div className="rounded-2xl p-5 relative overflow-hidden" {...fade(0)}
        style={{
          background: 'linear-gradient(140deg, rgba(5,150,105,0.38) 0%, rgba(16,185,129,0.14) 60%, rgba(6,182,212,0.08) 100%)',
          border: '1px solid rgba(16,185,129,0.18)',
          boxShadow: '0 0 60px rgba(5,150,105,0.10)',
        }}>
        <div className="absolute inset-0 backdrop-blur-2xl" />
        <div className="absolute -right-6 -top-6 w-44 h-44 rounded-full opacity-[0.08]"
          style={{ background: 'radial-gradient(circle, #34d399 0%, transparent 70%)' }} />
        <div className="relative z-10">
          <div className="flex items-start justify-between mb-5">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-widest mb-1 text-emerald-300/60">Financial Safety</p>
            ...
            <div className="text-right">
              ...
              <p className="text-[10px] text-white/30 uppercase tracking-wide font-semibold">saved this year</p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <div key={label} className="bg-white/[0.07] border border-white/[0.06] rounded-xl p-3 flex items-center gap-3">
      ...
      <Card delay={0.15} className="overflow-hidden">
        ...
        {TRANSACTIONS.map((tx, i) => (
          <motion.div key={tx.id}
            className={`px-5 py-3.5 flex items-center gap-3 ${i > 0 ? `border-t ${G.div}` : ''} ${tx.flagged ? 'bg-red-500/[0.05]' : ''}`}
            initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            transition={{ delay: 0.2 + i * 0.05 }}>
            <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm flex-shrink-0 ${tx.flagged ? 'bg-red-500/20 border border-red-500/30' : tx.positive ? 'bg-emerald-500/15 border border-emerald-500/20' : 'bg-white/[0.06] border border-white/[0.06]'}`}>
            ...
            <p className={`text-sm font-black flex-shrink-0 ${tx.positive ? 'text-emerald-400' : tx.flagged ? 'text-red-400' : 'text-white/55'}`}>
              {tx.amount}
            </p>
          </motion.div>
        ))}
    ...
    <div className="min-h-screen relative" style={{ background: BG }}>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute -top-32 -left-32 w-80 h-80 rounded-full"
          style={{ background: `radial-gradient(circle, ${GLOW_A} 0%, transparent 70%)`, filter: 'blur(48px)' }} />
        <div className="absolute top-[35%] -right-32 w-72 h-72 rounded-full"
          style={{ background: `radial-gradient(circle, rgba(16,185,129,0.07) 0%, transparent 70%)`, filter: 'blur(40px)' }} />
        <div className="absolute bottom-0 left-[25%] w-64 h-64 rounded-full"
          style={{ background: `radial-gradient(circle, ${GLOW_B} 0%, transparent 70%)`, filter: 'blur(40px)' }} />
      </div>
